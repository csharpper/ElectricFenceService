    ...
    public class FenceTrackMgr
    {
        ...
        public void Update()
        {
            do
            {
                ...
                var regions = ShipFenceMgr.Instance.Regions.Where(_ => _.Tracks.ContainsKey(trackId)).OrderBy(_=>!_.IsInner).ToArray();
                ...
                {//找到对应的区域列表
                    ...
                    for (int i = 0; i < regions.Count(); i++)
                    {
                        ...
                        // 此时，找到船舶对应最优先的区域，对该船进行下一步跟踪操作
                        //若为内部区域刷新，则推送完成后结束其他区域刷新
                        //若为外围区域刷新，则继续之后的刷新（因为可能同时在其他外围区域有事件，比如AB两个外围区域，进入A完成推送后可能仍有离开B的需求）
                        if (reg.UpdateTrack(gateTracks, trackId))
                            if(reg.IsInner)
                                break;
                    }
        ...
        private void removeInvalidTrack(IEnumerable<PolygonTrack> regions,string trackId)
        {
            ...
            {//遍历历史跟踪闸机，检查每个闸机所在区域是否可能在跟踪该船，若全不在跟踪，则说明该跟踪记录将结束
                ...
                if (regions.All(_ => _.GateIds.All(g => g != gateId)))//该船当前所有所在区域中均不含之前跟踪的闸机，即没有任何区域在监视这条船
                {
                    ...
                    Common.Log.Logger.Default.Trace($"结束闸机 {histrack.GateId} 对船舶 {histrack.ShipID} {histrack.Ship.Ship.Name} 的跟踪。是否尝试发送报文：{histrack.TrackStatus} {histrack.TrackStatus == GateTrackStatus.OuterTrack}");
        ...
        public bool UpdateTrack(GateTrackInfo track)
        {
